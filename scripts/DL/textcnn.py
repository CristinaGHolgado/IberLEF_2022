...
class CNNBert(nn.Module):
	...
	def forward(self, x, input_masks, token_type_ids):
		x = self.bert_model(x, attention_mask=input_masks, token_type_ids=token_type_ids)[2][-4:]#; print(x.shape)
...
def train(model, trainloader, valdataloader, criterion, args, device):
	...
	for epoch in range(args.EPOCHS):
		...
		model.eval()
		o,t, _ = evaluate(model, valdataloader,criterion, args, device)
		vlosses.append(_)
		if epoch > warmup:
...
if __name__ == '__main__':
	...
	if args.augment :
		...
		aug_data = pd.read_csv(f'/home/amsinha/IberLEF_2022/notebook/augment_{args.lclass}.csv', sep='\t')
		#print(aug_data.head())
	...
	model = CNNBert(args.lm, nclass, 768).to(device)
	...
	net = CNNBert(args.lm, nclass, 768).to(device)
	...
	load_and_run(df_test1, args, dtrain.label_encoder, net, name="devset")
	print('#'*20)
	print("*"*20,"Getting output on unseen test", "*"*20)
	df_test2 = load_data(args.test_file2, agg=0)
	load_and_run(df_test2, args, dtrain.label_encoder, net, no_labels=True, name="test")
